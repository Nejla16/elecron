...
function ProductDetailsPage() {
    ...
    useEffect(() => {
        favorite.find((el) => {
           if(el.id === parseInt(id)){ // ILI if(el.id == id) sa 2= samo vrijednost poredi a sa 3= moramo pretvoriti pomocu parseInt 
    ...
    //Ovdje dodajemo u nas CartSlice(jedan proizvod!)
    function productHandler() {
        dispatch(saveProductHandler(product))
    }

    //Ovdje cuvamo u favoriteSlice
  ...
  <div className="h-[100vh]">
    <div className="container mx-auto flex mt-[50px] gap-[20px]">
        ...
        <div className="w-[50%] flex flex-col">
            <img src={product.images?. [currentImage]} alt="" className="w-full h-[500px] object-cover border-2
             border-mainBlue p-3 rounded-[20px]"/>

            <div className="flex items-center justify-between mt-[20px]">
                {product.images?.map((img, index) => {
                    return <img key={index} src={img} className="w-[100px] h-[100px] border border-mainBlue p-2 rounded-[20px] cursor-pointer"
        ...
        <div className="w-[50%]">
            ...
            <div className="flex items-center gap-[10px]">
                ...
                <div className="flex items-center gap-2 text-green-500">
                    <FaCheck size={22}/> <span>In Stock</span>
                </div> 
                : 
            ...
            <div className="flex items-center gap-5 mt-[20px]">
                <Link to='/cartProducts' className="bg-mainYellow px-[24px] 
                py-[12px] rounded-full text-white"
                ...
                {/* Da hendlujemo */}
                {favoriteIcon === parseInt(id) ? <FaHeart size={32} color='red' /> : <CiHeart size={28} color="white"/>}
                </Link>
            </div>
